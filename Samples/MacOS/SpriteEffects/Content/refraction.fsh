...
uniform sampler2D DisplacementSampler_s1;
...
void main()
{
	...
	vec4 tex2 = texture2D(TextureSampler_s0, gl_TexCoord[0].xy);

	// Look up the displacement amount.
	vec4 displacement = texture2D(DisplacementSampler_s1, DisplacementScroll + gl_TexCoord[0].xy / 3.0);
	
	// Offset the main texture coordinates.
	vec2 offset = gl_TexCoord[0].xy;
	offset += displacement.xy * 0.2 - 0.109;

	gl_FragColor = texture2D(TextureSampler_s0, offset ) * gl_Color;
