    ...
    public sealed class IndirectPositionCollection : IList<Vector3>
    {
        ...
        public Vector3 this[int index]
        {
            get
            {
                ...
                return _geometry.Parent?.Positions[remap] ?? throw new Exception("Parent not set");
            }
            set => throw Readonly();
        }
        ...
        void ICollection<Vector3>.Add(Vector3 item) => throw Readonly();

        void ICollection<Vector3>.Clear() => throw Readonly();

        bool ICollection<Vector3>.Remove(Vector3 item) => throw Readonly();

        void IList<Vector3>.Insert(int index, Vector3 item) => throw Readonly();

        void IList<Vector3>.RemoveAt(int index) => throw Readonly();
