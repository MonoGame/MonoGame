...
namespace Microsoft.Xna.Framework.Content.Pipeline.Graphics
{
    ...
    /// </summary>
    public sealed class VertexChannel<T> : VertexChannel, IList<T>
    {
        ...
        internal override void InsertRange(int index, IEnumerable data)
        {
            ...
            if (data is not IEnumerable<T> ts)
                throw new ArgumentException(null, nameof(data));
            _items.InsertRange(index, ts);
        }
        ...
        public override IEnumerable<TargetType> ReadConvertedContent<TargetType>()
            => typeof(TargetType).IsAssignableFrom(typeof(T)) ? _items.Cast<TargetType>() : Convert<TargetType>(_items);
        ...
        private static IEnumerable<TargetType> Convert<TargetType>(IEnumerable<T> items)
        {
            ...
            if (!converter.CanConvertTo(typeof(TargetType)))
            {
                ...
                throw new NotImplementedException($"TypeConverter for {typeof(T).Name} -> {typeof(TargetType).Name} is not implemented.");
            }
        ...
        void ICollection<T>.Add(T value) =>((ICollection<T>)Items).Add(value);
        ...
        bool ICollection<T>.Remove(T value) => ((ICollection<T>)Items).Remove(value);
        ...
        void IList<T>.Insert(int index, T value) => Items.Insert(index, value);
        ...
        internal override void RemoveRange(int index, int count) => _items.RemoveRange(index, count);
    }
}
