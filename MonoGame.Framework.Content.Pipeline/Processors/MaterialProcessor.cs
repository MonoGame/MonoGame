...
namespace Microsoft.Xna.Framework.Content.Pipeline.Processors
{
    ...
    public class MaterialProcessor : ContentProcessor<MaterialContent, MaterialContent>
    {
        ...
        public virtual Color ColorKeyColor { get; set; } = Color.Magenta;
        ...
        public virtual MaterialProcessorDefaultEffect DefaultEffect { get; set; } = MaterialProcessorDefaultEffect.BasicEffect;
        ...
        public virtual bool GenerateMipmaps { get; set; } = false;
        ...
        /// Typically used to maximize compatability with a graphics card because many graphics cards do not support a material size that is not a power of two.
        ...
        [Description("""
            If enabled, the texture is resized to the next largest power of two, maximizing compatibility.
            Many graphics cards do not support texture sizes that are not a power of two.
            """)]
        public virtual bool ResizeTexturesToPowerOfTwo { get; set; } = true;

        /// <summary>
        /// Specifies the texture format of output materials. Materials can either be left unchanged from the source asset, converted to a corresponding Color,
        /// or compressed using the appropriate DxtCompressed format.
        /// </summary>
        ...
        [Description("""
                     Specifies the SurfaceFormat type of processed textures.
                     Textures can either remain unchanged from the source asset, converted to the Color format, or DXT compressed.
                     """)]
        public virtual TextureProcessorOutputFormat TextureFormat { get; set; } = TextureProcessorOutputFormat.Color;
        ...
        /// The EffectProcessor is used to process the EffectContent. Subclasses of MaterialProcessor can override this function to modify the parameters used
        /// to build EffectContent. For example, a different version of this function could request a different processor for the EffectContent.
        /// </remarks>
        protected virtual ExternalReference<CompiledEffectContent> BuildEffect(ExternalReference<EffectContent> effect, ContentProcessorContext context)
            => context.BuildAsset<EffectContent, CompiledEffectContent>(effect, new EffectImporter(), new EffectProcessor());
        ...
        /// <remarks>
        /// textureName can be used to determine which processor to use. For example, if a texture is being used as a normal map,
        ...
        /// <remarks>
        /// If the MaterialContent is of type EffectMaterialContent, a build is requested for Effect, and validation will be performed on the OpaqueData
        /// to ensure that all parameters are valid input to SetValue or SetValueTranspose. If the MaterialContent is a BasicMaterialContent,
        /// no validation will be performed on OpaqueData. Process requests builds for all textures in Textures, unless the MaterialContent is
        /// of type BasicMaterialContent, in which case a build will only be requested for DiffuseColor. The textures in Textures will be ignored.
        /// </remarks>
        ...
        public static MaterialContent CreateDefaultMaterial(MaterialProcessorDefaultEffect effect) => effect switch
            {
        ...
        public static MaterialProcessorDefaultEffect GetDefaultEffect(MaterialContent content) => content switch
            {
                AlphaTestMaterialContent => MaterialProcessorDefaultEffect.AlphaTestEffect,
                BasicMaterialContent => MaterialProcessorDefaultEffect.BasicEffect,
                DualTextureMaterialContent => MaterialProcessorDefaultEffect.DualTextureEffect,
                EnvironmentMapMaterialContent => MaterialProcessorDefaultEffect.EnvironmentMapEffect,
                SkinnedMaterialContent => MaterialProcessorDefaultEffect.SkinnedEffect,
                _ => throw new ArgumentOutOfRangeException(nameof(content), "Unknown material content type!")
            };
    }
}
