...
{
    class MetroFrameworkViewByDelegate<T> : IFrameworkView
        where T : Game
    {

        public MetroFrameworkViewByDelegate(Func<CoreApplicationView, IActivatedEventArgs, T> createGameInstanceDelegate)
        {
        ...
        private void ViewActivated(CoreApplicationView sender, IActivatedEventArgs args)
        {
            if (args.Kind == ActivationKind.Launch)
            {
                ...
                // Construct the game.
                _game = createGameInstanceDelegate(sender,args);
            }
