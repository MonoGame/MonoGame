...
namespace Microsoft.Xna.Framework.Content
{
    public partial class ContentManager : IDisposable
    {
        ...
        public virtual void Unload(string assetName)
        {
            ...
            // Accommodate key to loading practice
            var key = assetName.Replace('\\', '/');

            // Check if asset is loaded and start unloading if found
            object target = null;
            if (loadedAssets.TryGetValue(key, out target))
            {
                // Remove from loaded assets
                LoadedAssets.Remove(key);
                // Check if target is disposable
                if (target is IDisposable)
                {
                    // Remove from disposableAssets if already there
                    int i;
                    if ((i = disposableAssets.IndexOf((IDisposable)target)) >= 0)
        ...
        protected T ReadAsset<T>(string assetName, Action<IDisposable> recordDisposableObject)
        {
            ...
            // Cleanup
            // This step helps free the memory of leftover data
            // The effect is most noticable on WINRT
            GC.Collect();
