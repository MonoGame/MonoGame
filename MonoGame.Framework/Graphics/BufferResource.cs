...
namespace Microsoft.Xna.Framework.Graphics
{
    public partial class BufferResource : ShaderResource
    {
        ...
        protected Options BufferOptions { get; private set; }

        public BufferUsage BufferUsage { get; private set; }

		protected BufferResource(GraphicsDevice graphicsDevice, int elementCount, int elementStride, BufferUsage bufferUsage, Options bufferOptions)
		{
        ...
        /// </p>
        /// </remarks>
        public void GetData<T> (int offsetInBytes, T[] data, int startIndex, int elementCount, int elementStride = 0) where T : struct
        {
            var elementSizeInBytes = ReflectionHelpers.SizeOf<T>.Get();
            if (elementStride == 0)
                elementStride = elementSizeInBytes;

            var vertexByteSize = ElementCount * ElementStride;
            if (elementStride > vertexByteSize)
                throw new ArgumentOutOfRangeException("elementStride", "Element stride can not be larger than the buffer size.");
			...
			if (elementCount > 1 && elementCount * elementStride > vertexByteSize)
        ...
        protected void SetDataInternal<T>(int offsetInBytes, T[] data, int startIndex, int elementCount, int elementStride, SetDataOptions options) where T : struct
        {
            if (data == null)
                throw new ArgumentNullException("data");
            ...
            if (elementCount > 1 && (elementCount * elementStride > bufferSize))
                throw new InvalidOperationException("The element data is larger than the buffer.");
            if (elementStride < elementSizeInBytes)
                throw new ArgumentOutOfRangeException("The element stride must be greater than or equal to the size of the specified data (" + elementSizeInBytes + ").");            

            PlatformSetData<T>(offsetInBytes, data, startIndex, elementCount, elementStride, options, bufferSize, elementSizeInBytes);
        }
    }
}
