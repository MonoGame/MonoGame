...
namespace Microsoft.Xna.Framework.Graphics
{
	internal class SpriteBatchItem
	{
        ...
        public void Set(float x, float y, float dx, float dy, float w, float h, float sin, float cos, Color color, Vector2 texCoordTL, Vector2 texCoordBR)
        {
            ...
            vertexTR.Position.X = vertexTL.Position.X + wcos;
            vertexTR.Position.Y = vertexTL.Position.Y + wsin;
            vertexTR.Position.Z = Depth;
            vertexTR.Color = color;
            vertexTR.TextureCoordinate.X = texCoordBR.X;
            vertexTR.TextureCoordinate.Y = texCoordTL.Y;

            vertexBL.Position.X = vertexTL.Position.X - hsin;
            vertexBL.Position.Y = vertexTL.Position.Y + hcos;
            vertexBL.Position.Z = Depth;
            vertexBL.Color = color;
            vertexBL.TextureCoordinate.X = texCoordTL.X;
            vertexBL.TextureCoordinate.Y = texCoordBR.Y;

            vertexBR.Position.X = vertexTR.Position.X - hsin;
            vertexBR.Position.Y = vertexTR.Position.Y + hcos;
            vertexBR.Position.Z = Depth;
            vertexBR.Color = color;
            vertexBR.TextureCoordinate.X = texCoordBR.X;
            vertexBR.TextureCoordinate.Y = texCoordBR.Y;
