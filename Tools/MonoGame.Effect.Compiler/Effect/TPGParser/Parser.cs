    ...
    public partial class Parser 
    {
        ...
        private void ParsePass_Declaration(ParseNode parent) // NonTerminalSymbol: Pass_Declaration
        {
            ...
            if (tok.Type != TokenType.OpenBracket) {
             ...
             // Concat Rule
            tok = scanner.LookAhead(TokenType.VertexShader, TokenType.PixelShader, TokenType.HullShader, TokenType.DomainShader, TokenType.GeometryShader, TokenType.ComputeShader, TokenType.CullMode, TokenType.FillMode, TokenType.AlphaBlendEnable, TokenType.SrcBlend, TokenType.DestBlend, TokenType.BlendOp, TokenType.ColorWriteEnable, TokenType.DepthBias, TokenType.SlopeScaleDepthBias, TokenType.ZEnable, TokenType.ZWriteEnable, TokenType.ZFunc, TokenType.MultiSampleAntiAlias, TokenType.ScissorTestEnable, TokenType.StencilEnable, TokenType.StencilFail, TokenType.StencilFunc, TokenType.StencilMask, TokenType.StencilPass, TokenType.StencilRef, TokenType.StencilWriteMask, TokenType.StencilZFail); // ZeroOrMore Rule
                ...
                || tok.Type == TokenType.StencilZFail)
            {
                ...
                { // Choice Rule
                    ...
                    case TokenType.HullShader:
                        ParseHullShader_Pass_Expression(node); // NonTerminal Rule: HullShader_Pass_Expression
                        break;
                    case TokenType.DomainShader:
                        ParseDomainShader_Pass_Expression(node); // NonTerminal Rule: DomainShader_Pass_Expression
                        break;
                    case TokenType.GeometryShader:
                        ParseGeometryShader_Pass_Expression(node); // NonTerminal Rule: GeometryShader_Pass_Expression
                        break;
                    ...
                    case TokenType.StencilZFail:
                        ...
                        break;
                    default:
                        tree.Errors.Add(new ParseError("Unexpected token '" + tok.Text.Replace("\n", "") + "' found. Expected VertexShader, PixelShader, HullShader, DomainShader, GeometryShader, ComputeShader, CullMode, FillMode, AlphaBlendEnable, SrcBlend, DestBlend, BlendOp, ColorWriteEnable, DepthBias, SlopeScaleDepthBias, ZEnable, ZWriteEnable, ZFunc, MultiSampleAntiAlias, ScissorTestEnable, StencilEnable, StencilFail, StencilFunc, StencilMask, StencilPass, StencilRef, StencilWriteMask, or StencilZFail.", 0x0002, tok));
                        break;
                } // Choice Rule
            tok = scanner.LookAhead(TokenType.VertexShader, TokenType.PixelShader, TokenType.HullShader, TokenType.DomainShader, TokenType.GeometryShader, TokenType.ComputeShader, TokenType.CullMode, TokenType.FillMode, TokenType.AlphaBlendEnable, TokenType.SrcBlend, TokenType.DestBlend, TokenType.BlendOp, TokenType.ColorWriteEnable, TokenType.DepthBias, TokenType.SlopeScaleDepthBias, TokenType.ZEnable, TokenType.ZWriteEnable, TokenType.ZFunc, TokenType.MultiSampleAntiAlias, TokenType.ScissorTestEnable, TokenType.StencilEnable, TokenType.StencilFail, TokenType.StencilFunc, TokenType.StencilMask, TokenType.StencilPass, TokenType.StencilRef, TokenType.StencilWriteMask, TokenType.StencilZFail); // ZeroOrMore Rule
            }
        ...
        private void ParseHullShader_Pass_Expression(ParseNode parent) // NonTerminalSymbol: HullShader_Pass_Expression
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(scanner.GetToken(TokenType.HullShader_Pass_Expression), "HullShader_Pass_Expression");
            parent.Nodes.Add(node);


             // Concat Rule
            tok = scanner.Scan(TokenType.HullShader); // Terminal Rule: HullShader
            n = node.CreateNode(tok, tok.ToString() );
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
            if (tok.Type != TokenType.HullShader) {
                tree.Errors.Add(new ParseError("Unexpected token '" + tok.Text.Replace("\n", "") + "' found. Expected " + TokenType.HullShader.ToString(), 0x1001, tok));
                return;
            }
            ...
            parent.Token.UpdateRange(node.Token);
        } // NonTerminalSymbol: HullShader_Pass_Expression

        private void ParseDomainShader_Pass_Expression(ParseNode parent) // NonTerminalSymbol: DomainShader_Pass_Expression
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(scanner.GetToken(TokenType.DomainShader_Pass_Expression), "DomainShader_Pass_Expression");
            parent.Nodes.Add(node);


             // Concat Rule
            tok = scanner.Scan(TokenType.DomainShader); // Terminal Rule: DomainShader
            n = node.CreateNode(tok, tok.ToString() );
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
            if (tok.Type != TokenType.DomainShader) {
                tree.Errors.Add(new ParseError("Unexpected token '" + tok.Text.Replace("\n", "") + "' found. Expected " + TokenType.DomainShader.ToString(), 0x1001, tok));
                return;
            }
            ...
            parent.Token.UpdateRange(node.Token);
        } // NonTerminalSymbol: DomainShader_Pass_Expression

        private void ParseGeometryShader_Pass_Expression(ParseNode parent) // NonTerminalSymbol: GeometryShader_Pass_Expression
        {
            Token tok;
            ParseNode n;
            ParseNode node = parent.CreateNode(scanner.GetToken(TokenType.GeometryShader_Pass_Expression), "GeometryShader_Pass_Expression");
            parent.Nodes.Add(node);


             // Concat Rule
            tok = scanner.Scan(TokenType.GeometryShader); // Terminal Rule: GeometryShader
            n = node.CreateNode(tok, tok.ToString() );
            node.Token.UpdateRange(tok);
            node.Nodes.Add(n);
            if (tok.Type != TokenType.GeometryShader) {
                tree.Errors.Add(new ParseError("Unexpected token '" + tok.Text.Replace("\n", "") + "' found. Expected " + TokenType.GeometryShader.ToString(), 0x1001, tok));
                return;
            }
            ...
            parent.Token.UpdateRange(node.Token);
        } // NonTerminalSymbol: GeometryShader_Pass_Expression
