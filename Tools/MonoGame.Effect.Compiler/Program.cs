            ...
            try
            {
                ...
                if (writeHeader)
                {
                    ...
                    using (var binary = new MemoryStream())
                    {
                        ...
                        string displaySourceFilepath = sourceFilepath;
                        if (!string.IsNullOrEmpty(options.RelativeRoot))
                        {
                            try
                            {
                                var root = Path.GetFullPath(options.RelativeRoot);
                                var rel = Path.GetRelativePath(root, sourceFilepath);
                                displaySourceFilepath = rel.Replace('\\', '/');
                            }
                            catch
                            {
                                Console.Error.WriteLine("Failed to make the source file path relative to '{0}'!", options.RelativeRoot);
                            }
                        }
                        // Write the header.
