...
namespace MonoGame.Tools.Pipeline
{
    public partial class ProjectControl : Pad
    {
        ...
        private void TreeView_SelectedItemChanged(object sender, EventArgs e)
        {
            ...
            var selitems = _treeView.SelectedItems;

            foreach (var selitem in selitems)
            {
                var item = selitem as TreeGridItem;
        ...
        private void SetExists(TreeGridItem titem, bool exists)
        {
            ...
            if (item is DirectoryItem)
            {
                ...
                titem.SetValue(0, Global.GetDirectoryIcon(fex));
            }
            else
                titem.SetValue(0, Global.GetFileIcon(PipelineController.Instance.GetFullPath(item.OriginalPath), exists));
        ...
        private TreeGridItem GetorAddItem(TreeGridItem root, IProjectItem item)
        {
            ...
            if (item is DirectoryItem)
                ret.SetValue(0, Global.GetDirectoryIcon(item.Exists));
            else
                ret.SetValue(0, Global.GetFileIcon(PipelineController.Instance.GetFullPath(item.OriginalPath), item.Exists));
