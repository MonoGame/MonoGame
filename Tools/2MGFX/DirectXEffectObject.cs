...
namespace TwoMGFX
{
    internal partial class DirectXEffectObject : EffectObject {
        ...
        public readonly bool MojoShader;

        public DirectXEffectObject(bool mojoShader)
        {
            MojoShader = mojoShader;
        }

        public override void Build(ShaderInfo shaderInfo, out string errorsAndWarnings)
        {
            var profile = MojoShader ? ShaderProfile.OpenGL : ShaderProfile.DirectX_11;

            errorsAndWarnings = string.Empty;

            // These are filled out as we process stuff.
            ConstantBuffers = new List<ConstantBufferData>();
            Shaders = new List<ShaderData>();

            // Go thru the techniques and that will find all the 
            // shaders and constant buffers.
            Techniques = new d3dx_technique[shaderInfo.Techniques.Count];
            for (var t = 0; t < shaderInfo.Techniques.Count; t++)
            {
                ...
                for (var p = 0; p < tinfo.Passes.Count; p++)
                {
                    ...
                    {
                        pass.state_count += 1;
                        tempstate[pass.state_count - 1] = CreateShader(this, shaderInfo, profile, pinfo.psFunction, pinfo.psModel, false, ref errorsAndWarnings);
                    }

                    if (!string.IsNullOrEmpty(pinfo.vsFunction))
                    {
                        pass.state_count += 1;
                        tempstate[pass.state_count - 1] = CreateShader(this, shaderInfo, profile, pinfo.vsFunction, pinfo.vsModel, true, ref errorsAndWarnings);
                    }
            ...
            for (var c = 0; c < ConstantBuffers.Count; c++)
            {
                var cb = ConstantBuffers[c];
            ...
            foreach (var shader in Shaders)
            {
                for (var s = 0; s < shader._samplers.Length; s++)
                {
                    ...
                    if (match == -1)
                    {
                        ...
                        switch (sampler.type)
                        {
                            case TwoMGFX.MojoShader.MOJOSHADER_samplerType.MOJOSHADER_SAMPLER_1D:
                                param.type = D3DXPARAMETER_TYPE.TEXTURE1D;
                                break;

                            case TwoMGFX.MojoShader.MOJOSHADER_samplerType.MOJOSHADER_SAMPLER_2D:
                                param.type = D3DXPARAMETER_TYPE.TEXTURE2D;
                                break;

                            case TwoMGFX.MojoShader.MOJOSHADER_samplerType.MOJOSHADER_SAMPLER_VOLUME:
                                param.type = D3DXPARAMETER_TYPE.TEXTURE3D;
                                break;

                            case TwoMGFX.MojoShader.MOJOSHADER_samplerType.MOJOSHADER_SAMPLER_CUBE:
        ...
        public d3dx_state CreateShader(DirectXEffectObject effect, ShaderInfo shaderInfo, ShaderProfile profile, 
            string shaderFunction, string shaderModel, bool isVertexShader, ref string errorsAndWarnings)
        {
            // Compile and create the shader.
            var bytecode = CompileHLSL(shaderInfo, profile, shaderFunction, shaderModel, ref errorsAndWarnings);

            ShaderData shaderData;
            // First look to see if we already created this same shader.
            if (!effect.GetCachedShader(bytecode, out shaderData))
            {
                if (profile == ShaderProfile.DirectX_11)
                    shaderData = ShaderData.CreateHLSL(bytecode, isVertexShader, effect.ConstantBuffers, effect.Shaders.Count, shaderInfo.SamplerStates, shaderInfo.Debug);
                else
                    // TODO this needs to be removed when the GLFX path can replace MojoShader
                    shaderData = ShaderData.CreateGLSL(bytecode, isVertexShader, effect.ConstantBuffers, effect.Shaders.Count, shaderInfo.SamplerStates, shaderInfo.Debug);
            ...
            state.operation = isVertexShader ? 146 : (uint)147;
        ...
        internal bool GetCachedShader(byte[] bytecode, out ShaderData data)
        {
            foreach (var shader in Shaders)
            {
        ...
        public d3dx_parameter[] Parameters { get; set; }

        public d3dx_technique[] Techniques { get; set; }

        public List<ShaderData> Shaders { get; set; }

        public List<ConstantBufferData> ConstantBuffers { get; set; }
    }
}