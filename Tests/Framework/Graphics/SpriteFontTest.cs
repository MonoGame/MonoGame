...
But the answer was still '42'.
        ...
        [Test]
        [RunOnUI]
        public void Runtime_FromFile_PathIsNull_ThrowsArgumentNullException()
        {
            Assert.Throws<ArgumentNullException>(() => SpriteFont.FromFile(gd, null, 32, Array.Empty<CharacterRegion>()));
        }

        [Test]
        [RunOnUI]
        public void Runtime_FromStream_GraphicsDeviceIsNull_ThrowsArgumentNullException()
        {
            using (Stream stream = OpenRuntimeFontStream())
            {
                Assert.Throws<ArgumentNullException>(() => SpriteFont.FromStream(null, stream, 32, Array.Empty<CharacterRegion>()));
            }
        }

        [Test]
        [RunOnUI]
        public void Runtime_FromStream_StreamIsNull_ThrowsArgumentNullException()
        {
            Assert.Throws<ArgumentNullException>(() => SpriteFont.FromStream(gd, null, 32, Array.Empty<CharacterRegion>()));
        }

        [Test]
        [RunOnUI]
        public void Runtime_FromStream_SizeIsZero_ThrowsArgumentOutOfRangeException()
        {
            using (Stream stream = OpenRuntimeFontStream())
            {
                Assert.Throws<ArgumentOutOfRangeException>(() => SpriteFont.FromStream(gd, stream, 0, Array.Empty<CharacterRegion>()));
            }
        }

        [Test]
        [RunOnUI]
        public void Runtime_FromStream_CharacterRegionsIsNull_ThrowsArgumentNullException()
        {
            using (Stream stream = OpenRuntimeFontStream())
            {
                Assert.Throws<ArgumentNullException>(() => SpriteFont.FromStream(gd, stream, 32, null));
            }
        }

        [Test]
        [RunOnUI]
        public void Runtime_FromStream_EmptyCharacterRegions_ReturnsSpriteFont()
        {
            using (Stream stream = OpenRuntimeFontStream())
            {
                SpriteFont font = SpriteFont.FromStream(gd, stream, 32, Array.Empty<CharacterRegion>());
        ...
        [Test]
        [RunOnUI]
        public void Runtime_MeasureString_WithoutPreloadedGlyphs_ReturnsPositiveWidth()
        {
            using (Stream stream = OpenRuntimeFontStream())
            {
                SpriteFont font = SpriteFont.FromStream(gd, stream, 32, Array.Empty<CharacterRegion>());
        ...
        [Test]
        [RunOnUI]
        public void Runtime_DrawString_WithoutPreloadedGlyphs_DoesNotThrow()
        {
            using (Stream stream = OpenRuntimeFontStream())
            {
                SpriteFont font = SpriteFont.FromStream(gd, stream, 32, Array.Empty<CharacterRegion>());
                ...
                _spriteBatch.Begin();
                Assert.DoesNotThrow(() => _spriteBatch.DrawString(font, "abc", Vector2.Zero, Color.White));
                _spriteBatch.End();
            }
        }
        ...
        [Test]
        [RunOnUI]
        public void Runtime_MeasureString_WithDefaultCharacter_DoesNotThrow()
        {
            using (Stream stream = OpenRuntimeFontStream())
            {
                List<CharacterRegion> characterRegions = new List<CharacterRegion>
                {
                    new CharacterRegion('A', 'A')
                };

                SpriteFont font = SpriteFont.FromStream(gd, stream, 32, characterRegions);
                font.DefaultCharacter = 'A';

                Assert.DoesNotThrow(() => font.MeasureString("B"));
            }
        }
